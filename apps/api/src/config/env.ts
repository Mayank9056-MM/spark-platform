...
const envSchema = z
  .object({
    ...
    RESEND_API_KEY: z.preprocess(emptyToUndefined, z.string().trim().min(1).optional()),
    // Sender address only (no display name). It must belong to a domain verified
    // in Resend; this schema cannot check that.
  ...
  .refine((e) => e.NODE_ENV !== 'production' || e.RESEND_API_KEY !== undefined, {
    path: ['RESEND_API_KEY'],
    error: 'RESEND_API_KEY is required when NODE_ENV=production',
  })
  .refine((e) => e.NODE_ENV !== 'production' || e.RESEND_FROM_EMAIL !== undefined, {
    path: ['RESEND_FROM_EMAIL'],
    error: 'RESEND_FROM_EMAIL is required when NODE_ENV=production',
  })
  .refine((e) => e.NODE_ENV !== 'production' || e.APP_URL.startsWith('https://'), {
    path: ['APP_URL'],
    error: 'APP_URL must use https when NODE_ENV=production',
  });
