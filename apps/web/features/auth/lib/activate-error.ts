...
const SERVER_MESSAGE = "Activation isn't available right now. Try again in a few minutes.";
const FALLBACK_MESSAGE = 'Activation failed. Try again.';

/**
 * The API answers 400 TOKEN_INVALID for a token that is unknown, expired,
 * already used, or of the wrong purpose. It deliberately does not say which,
 * so neither does the UI. Retrying with the same link cannot succeed.
 */
export function isInvalidActivationTokenError(error: unknown): boolean {
  return error instanceof ApiClientError && error.code === API_ERROR_CODE.TOKEN_INVALID;
}

/** Copy for every failure the form can show while the link is still worth retrying. */
export function getActivateErrorMessage(error: unknown): string {
